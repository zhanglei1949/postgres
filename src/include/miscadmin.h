 ...
 *		globals.h						global
 *variables
 *		pdir.h							directory path
 *crud pinit.h							postgres
 *initialization pmod.h processing modes Over time, this has also become the
 *preferred place for widely known resource-limitation stuff, such as work_mem
 *and check_stack_depth().
 *
...
/*
 * IntervalStyles
 *	 INTSTYLE_POSTGRES			   Like Postgres < 8.4 when
 *DateStyle = 'iso' INTSTYLE_POSTGRES_VERBOSE	   Like Postgres < 8.4 when
 *DateStyle != 'iso' INTSTYLE_SQL_STANDARD		   SQL standard interval
 *literals INTSTYLE_ISO_8601			   ISO-8601-basic formatted
 *intervals
 */
...
extern void InitPostgres(const char* in_dbname, Oid dboid, const char* username,
                         Oid useroid, bits32 flags, char* out_dbname);
